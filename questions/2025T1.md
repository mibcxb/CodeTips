...
```java
    public static int maxExtremeSum(int[] nums, int k) {
        Arrays.sort(nums);
        int sum = 0;
        for (int i = 0; i < k; i++) {
            sum += nums[k - i - 1] - nums[i];
        }
        return sum;
    }
```
...
给妈妈买花，给定一个数组代表每种花的支数，每种花只能买奇数支，或者不买，
最终买完的支数也要是奇数，问最多能买多少支花。如:3，{5,5,8}，最多买 17
朵，5+5+7，因为 8 支是偶数，所以买 7 支。
